...
def plot_data_from_dict(
    exp_data, prob_info, open_loop=False, CEM=False, fig_objs={}, label=""
):
    """
    ...
    if open_loop:
        ...
        (ax11, ax12, ax21, ax22) = fig_objs["axes"]
        ax11.plot(np.arange(len(Tplot)) * ts, Tplot, ":", label=label)

        ax12.plot(np.arange(len(Iplot)) * ts, Iplot, "--", label=label)

        ax21.step(
            np.arange(len(exp_data["Psave"])) * ts, exp_data["Psave"], label=label
        )

        ax22.step(
            np.arange(len(exp_data["qSave"])) * ts, exp_data["qSave"], label=label
        )
        ...
        if CEM:
            ...
            ax21.step(
                np.arange(st - 1) * ts, exp_data["Psave"][: (st - 1)], label=label
            )

            ax22.step(
                np.arange(st - 1) * ts, exp_data["qSave"][: (st - 1)], label=label
            )
...
def plot_data_from_file(
    datafile, prob_info, open_loop=False, CEM=False, fig_objs={}, label=""
):
    """
